...
auto IndexInfo::LookupInteger(int32_t key) const -> const std::vector<RID> * {
  if (int_index_ != nullptr) {
    std::vector<RID> ignored;
    if (!int_index_->GetValue(key, &ignored)) {
      return nullptr;
    }
  }
...
void IndexInfo::InsertEntry(int32_t key, const RID &rid) {
  auto &rids = int_rid_map_[key];
  if (rids.empty() && int_index_ != nullptr) {
    if (!int_index_->Insert(key, rid)) {
      int_index_.reset();
    }
  }
  rids.push_back(rid);
...
void IndexInfo::RemoveEntry(int32_t key, const RID &rid) {
  auto it = int_rid_map_.find(key);
  if (it == int_rid_map_.end()) {
    return;
  }
...
auto Catalog::CreateIndex(const std::string &index_name, const std::string &table_name,
  ...
  if (key_attrs.size() == 1 &&
    ...
    index_info->int_index_ =
        std::make_unique<BPlusTree<int, RID, std::less<int>>>(index_name, bpm_, std::less<int>{});
